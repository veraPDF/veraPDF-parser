...
public class PDResourcesHandler {
	...
	public PDColorSpace getColorSpace(COSName name, boolean checkUndefined) {
		if (name != null) {
			return getColorSpace(name.getName(), checkUndefined);
		}
		return null;
	}
	...
	public PDColorSpace getColorSpace(ASAtom name, boolean checkUndefined) {
		...
		if (isDefaultColorSpaceUsed(name)) {
			if (objectResources != null) {
				colorSpace = objectResources.getDefaultColorSpace(name);
			} else if (pageResources != null) {
				colorSpace = pageResources.getDefaultColorSpace(name);
			} else {
				colorSpace = EMPTY.pageResources.getDefaultColorSpace(name);
			}
		} else {
			if (this.objectResources != null) {
				...
				colorSpace = EMPTY.pageResources.getColorSpace(name);
			}
		}
