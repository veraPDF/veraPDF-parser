    ...
    public static byte[] decodePredictor(int predictor, int colors, int bitsPerComponent,
        ...
        if (predictor == 1) {
            ...
            while (inputPointer < input.length) {
                ...
                // Read line
                int actlineEnd = Math.min(inputPointer + rowlength, input.length);
                actline = Arrays.copyOfRange(input, inputPointer, actlineEnd);
                inputPointer += rowlength;
                ...
                switch (linePredictor) {
                    case 2:
                        ...
                        if (bitsPerComponent != 8) {
                            throw new RuntimeException("TIFF-Predictor with " + bitsPerComponent
